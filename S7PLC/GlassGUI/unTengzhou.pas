...
uses
  ...
  CodeSiteLogging, RxCtrls, Vcl.ExtCtrls, RxHook;
...
procedure TfrmTengzhou.DrawAirDesk(areaNo: byte; Sx, Sy, xLength, yLength, xHole, yHole, radial, direction, vOffset : integer; zoom :word=100);
var
  iType, iState: UINT;
  iRect: TRect;
...
procedure TfrmTengzhou.DrawHorizionStacker(areaNo: byte; Sx, Sy, Rlength, Rwidth, vOffset :word; borderColor, fillColor :TColor);
var
...
procedure TfrmTengzhou.DrawSnap(Sx, Sy, Rlength : word;
...
procedure TfrmTengzhou.BitBtn12Click(Sender: TObject);
begin
...
procedure TfrmTengzhou.BitBtn13Click(Sender: TObject);
begin
  Canvas.Brush.Color := clWhite;
...
procedure TfrmTengzhou.BitBtn14Click(Sender: TObject);
var
  bmp :TBitMap ;
  cvs :TCanvas;
begin
...
procedure TfrmTengzhou.DrawChevron(Sx, Sy, Rlength, Rwidth, Rratio: word;
  borderColor, fillColor: TColor; Shorizon: boolean);
var
...
procedure TfrmTengzhou.DrawEdgeTrim(Sx: word; Sy: word; Rlength: word;
...
procedure TfrmTengzhou.BitBtn11Click(Sender: TObject);
begin
  DrawEdgeTrim(300, 200, 4455, 4500, 10, 14, clGray, clMenu, True);
  // DrawEdgeTrim(1000, 200, 4455, 4500, 10, 14, clGray, clMenu, False);
end;

procedure TfrmTengzhou.DrawRollers(Sx, Sy, Rlength, Rwidth, Rratio,
    ...
    begin
      ...
      begin
        ...
        begin
          ...
          //DrawChevron(Cx, Cy, 630, Trunc(rSpacing * 0.4), Rratio, clBtnShadow, clScrollBar, Shorizon);
        end;
      end
      else
...
procedure TfrmTengzhou.DrawRoller(Sx, Sy, Slength, Swidth, Sratio,
...
procedure TfrmTengzhou.BitBtn10Click(Sender: TObject);
begin
  Canvas.Brush.Color := clWhite;
  Canvas.FillRect(Rect(0, 0, self.width - 1, self.height - 1));
...
procedure TfrmTengzhou.DrawPositonRect(areaNo: byte);
var
...
begin
  ...
  begin
    ...
    begin
      DrawRect(iStartX + vOffset * 2, iStartY - vOffset, iStartX + vOffset * 3,
        iStartY + posLabel[i].wid + vOffset, clBlue, clBlue);
      DrawRect(iStartX + vOffset * 6, iStartY - vOffset, iStartX + vOffset * 7,
        iStartY + posLabel[i].wid + vOffset, clBlue, clBlue);
      if areaNo = 0 then
      begin
        DrawRect(iStartX + vOffset * 2, iStartY + layoutDistance - vOffset,
          iStartX + vOffset * 3, iStartY + layoutDistance + posLabel[i].wid +
          vOffset, clBlue, clBlue);
        DrawRect(iStartX + vOffset * 6, iStartY + layoutDistance - vOffset,
          iStartX + vOffset * 7, iStartY + layoutDistance + posLabel[i].wid +
          vOffset, clBlue, clBlue);
      end;
    end;
    ...
    begin
      ...
      Canvas.Polygon([point(iStartX + vOffset * 2, iStartY - vOffset),
        point(iStartX + vOffset * 4, iStartY + posLabel[i].wid + vOffset),
        point(iStartX + vOffset * 5, iStartY + posLabel[i].wid + vOffset),
        point(iStartX + vOffset * 3, iStartY - vOffset)]);
      Canvas.Polygon([point(iStartX + vOffset * 5, iStartY - vOffset),
        point(iStartX + vOffset * 7, iStartY + posLabel[i].wid + vOffset),
        point(iStartX + vOffset * 8, iStartY + posLabel[i].wid + vOffset),
        point(iStartX + vOffset * 6, iStartY - vOffset)]);
      if areaNo = 0 then
      begin
        Canvas.Polygon([point(iStartX + vOffset * 2, iStartY + layoutDistance -
          vOffset), point(iStartX + vOffset * 4, iStartY + layoutDistance +
          posLabel[i].wid + vOffset), point(iStartX + vOffset * 5,
          iStartY + layoutDistance + posLabel[i].wid + vOffset),
          point(iStartX + vOffset * 3, iStartY + layoutDistance - vOffset)]);
        Canvas.Polygon([point(iStartX + vOffset * 5, iStartY + layoutDistance -
          vOffset), point(iStartX + vOffset * 7, iStartY + layoutDistance +
          posLabel[i].wid + vOffset), point(iStartX + vOffset * 8,
          iStartY + layoutDistance + posLabel[i].wid + vOffset),
          point(iStartX + vOffset * 6, iStartY + layoutDistance - vOffset)]);
      end;
    end;

    if (posLabel[i].Num = '130.026') then
    begin
...
//          clBlue, clBlue);
    ...
    if (posLabel[i].Num = '200.010') then
      DrawAirDesk(areaNo, iStartX, iStartY, posLabel[i].len, posLabel[i].wid, 5, 4, 4, 0, vOffset);
//    begin
//      Canvas.Brush.Color := clMedGray;
//      if posLabel[i].gap = 1 then
...
//          then
//          begin
//            Canvas.Ellipse(iStartX + m - vOffset, iStartY + n - vOffset,
//              iStartX + m + vOffset, iStartY + n + vOffset);
//            if areaNo = 0 then
//              Canvas.Ellipse(iStartX + m - vOffset, iStartY + layoutDistance + n
//                - vOffset, iStartX + m + vOffset, iStartY + layoutDistance + n
//                + vOffset);
//          end;
//
//    end;

    if (posLabel[i].Num = '170.030') or (posLabel[i].Num = '170.060') or
      (posLabel[i].Num = '170.090') or (posLabel[i].Num = '170.140') or
      (posLabel[i].Num = '170.150') then
        DrawHorizionStacker(areaNo, iStartX, iStartY, posLabel[i].Len, posLabel[i].wid, vOffset, clGreen, clRed);

    if (posLabel[i].Num = '170.030') or (posLabel[i].Num = '170.060') or (posLabel[i].Num = '170.090') then
    begin
...
procedure TfrmTengzhou.DrawLabel;
var
  i: byte;
  iType, iState: UINT;
  iRect: TRect;
...
procedure TfrmTengzhou.DrawConnection(areaNo: byte);
var
  i: integer;
  iType, iState: UINT;
...
begin
  ...
  posData[19].Num := '170.010';
  posData[19].Len := 4200;
  posData[19].wid := 4500;
  posData[19].NumPos := 130;

  posData[20].Num := '170.020';
  posData[20].Len := 4200;
  posData[20].wid := 4500;
  posData[20].NumPos := 160;

  posData[21].Num := '170.030';
  posData[21].Len := 4500;
  posData[21].wid := 4500;
  posData[21].NumPos := 190;

  posData[22].Num := '170.040';
  posData[22].Len := 4500;
  posData[22].wid := 4500;
  posData[22].NumPos := 160;

  posData[23].Num := '170.050';
  posData[23].Len := 4500;
  posData[23].wid := 4500;
  posData[23].NumPos := 130;

  posData[24].Num := '170.060';
  posData[24].Len := 4500;
  posData[24].wid := 4500;
  posData[24].NumPos := 100;

  posData[25].Num := '170.070';
  posData[25].Len := 4500;
  posData[25].wid := 4500;
  posData[25].NumPos := 130;

  posData[26].Num := '170.080';
  posData[26].Len := 4500;
  posData[26].wid := 4500;
  posData[26].NumPos := 160;

  posData[27].Num := '170.090';
  posData[27].Len := 4500;
  posData[27].wid := 4500;
  posData[27].NumPos := 190;

  posData[28].Num := '170.100';
  posData[28].Len := 4500;
  posData[28].wid := 4500;
  posData[28].NumPos := 160;

  posData[29].Num := '170.110';
  posData[29].Len := 4500;
  posData[29].wid := 4500;
  posData[29].NumPos := 130;

  posData[30].Num := '170.120';
  posData[30].Len := 4500;
  posData[30].wid := 4500;
  posData[30].NumPos := 100;

  posData[31].Num := '170.130';
  posData[31].Len := 4500;
  posData[31].wid := 4500;
  posData[31].NumPos := 130;

  posData[32].Num := '170.140';
  posData[32].Len := 5400;
  posData[32].wid := 4500;
  posData[32].NumPos := 160;

  posData[33].Num := '170.150';
  posData[33].Len := 5400;
  posData[33].wid := 4500;
  posData[33].NumPos := 190;

  posData[34].Num := '170.160';
  posData[34].Len := 5400;
  posData[34].wid := 4500;
  posData[34].NumPos := 160;

  posData[35].Num := '170.170';
  posData[35].Len := 5400;
  posData[35].wid := 4500;
...
procedure TfrmTengzhou.DrawLine(Sx, Sy, Ex, Ey: word; CurColor: TColor);
begin
...
procedure TfrmTengzhou.DrawRect(Sx, Sy, Ex, Ey: word; borderColor: TColor;
  fillColor: TColor = clWhite);
begin
  Canvas.Pen.Color := borderColor;
  Canvas.Brush.Color := fillColor;
...
procedure TfrmTengzhou.DrawArrow(Sx, Sy: word; sizeLength: word; direction: byte);
var
  Ex, Ey: word;
  X1, Y1, X2, Y2: word;
begin
...
procedure TfrmTengzhou.DrawArrow(Sx, Sy: word; sizeLength, sizeWidth: word;
  direction: byte);
var
  Ex, Ey: word;
...
procedure TfrmTengzhou.BitBtn2Click(Sender: TObject);
begin
...
procedure TfrmTengzhou.DrawArea(areaNo: byte);
var
  i: word;
begin
...
procedure TfrmTengzhou.BitBtn1Click(Sender: TObject);
var
