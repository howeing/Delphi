...
uses
  ...
  Data.Bind.EngExt, Vcl.Bind.DBEngExt, System.Rtti, System.Bindings.Outputs, unPublic, System.IOUtils ,
  Vcl.Bind.Editors, Data.Bind.Components, Xml.xmldom, Xml.XMLIntf, Xml.XMLDoc, IpEdit,
  Vcl.Menus, CodeSiteLogging, DBGridEhGrouping, ToolCtrlsEh, DBGridEhToolCtrls,
...
var
  plcfile, tagfile, xmlfile :string;
begin
  reConnect :=False;
  ...
  xmlfile :=GetAppPath + '\config.xml';
  if not TFile.Exists(xmlfile) then
  begin
    Application.MessageBox(PChar(xmlfile + ' 丢失!'), '文件丢失' ,MB_OK);
    Application.Terminate;
  end;

  bxml.FileName :=xmlfile;
  ...
  cs1.SendXmlFile('Config.xml', bxml.FileName);
end;
