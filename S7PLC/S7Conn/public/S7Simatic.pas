...
type
  ...
  public
    ...
    validBlocks :integer;                                                        //numbers of DB block list
    minDBno, maxDBno :integer;
...
var
  ...
  blockNumList: array [0 .. DBCounter - 1] of word; // PS7BlocksOfType;
begin
  size := SizeOf(Buffer);
    ...
    if Check(res, 'DB List in AG') then
      if iCount > 0 then
        for i := 0 to iCount - 1 do
          if (blockNumList[i] { BlockItem[i].no } >= minDBno) and
            ((blockNumList[i] { BlockItem[i].no } <= maxDBno)) then
          begin
